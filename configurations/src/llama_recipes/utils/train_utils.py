...
def train(model, train_dataloader,eval_dataloader, tokenizer, optimizer, lr_scheduler, gradient_accumulation_steps, train_config, fsdp_config=None, local_rank=None, rank=None, wandb_run=None):
    ...
    for epoch in range(train_config.num_epochs):
        ...
        if train_config.run_validation and train_config.early_stopping_patience >= 0:
            if epochs_since_improvement > train_config.early_stopping_patience:
                if not train_config.enable_fsdp or rank == 0:
                    print(f"Early stopping triggered at epoch {epoch+1}: no validation improvement for {epochs_since_improvement} consecutive epoch(s).")
                if train_config.enable_fsdp:
                    dist.barrier()
