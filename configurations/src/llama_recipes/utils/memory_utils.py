...
class MemoryTrace:
    ...
    def save_stats(self, train_config):
        device_str = None
        if is_xpu_available():
            device_str = "XPU"
        elif torch.cuda.is_available():
        ...
        if device_str:
            with open(f"{train_config.output_dir}/training.log", "a") as f:
                f.write(
                    f"Max {device_str} memory allocated was {self.peak} GB\n"
                    f"Max {device_str} memory reserved was {self.max_reserved} GB\n"
                    f"Peak active {device_str} memory was {self.peak_active_gb} GB\n"
                    f"{device_str} Malloc retries : {self.malloc_retries}\n"
                )
        with open(f"{train_config.output_dir}/training.log", "a") as f:
            f.write(f"CPU Total Peak Memory consumed during the train (max): {self.cpu_peaked + self.cpu_begin} GB\n")
